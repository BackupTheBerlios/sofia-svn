...
(*                                                                              *)
(********************************************************************************)
(* $Id: jvuibsynedit.pas,v 1.8 2005/03/21 22:07:38 progdigy Exp $ *)
(********************************************************************************)
...
const
  ...
  SQLToKens: array[0..261] of string =
   ('ACTION','ACTIVE','ADD','ADMIN','AFTER','ALL','ALTER','AND','ANY','AS','ASC',
    'ASCENDING','AT','AUTO','AVG','BACKUP','BASE_NAME','BEFORE','BEGIN','BETWEEN',
    'BIGINT','BLOB', 'BLOCK', 'BOOLEAN','BREAK','BY','CACHE','CASCADE','CASE','CAST','CHAR',
    'CHARACTER','CHECK','CHECK_POINT_LENGTH','CLOSE','COALESCE','COLLATE','COLUMN',
    'COMMIT','COMMITTED','COMPUTED','CONDITIONAL','CONSTRAINT','CONTAINING','COUNT','CREATE', 'CROSS',
    ...
    'DATE','DAY','DEBUG','DEC','DECIMAL','DECLARE','DEFAULT','DELETE','DELETING',
    'DESC','DESCENDING','DESCRIPTOR','DIFFERENCE','DISTINCT','DO','DOMAIN','DOUBLE',
    'DROP','ELSE','END','ENTRY_POINT','ESCAPE','EXCEPTION','EXECUTE','EXISTS','EXIT',
    'EXTERNAL','EXTRACT','FALSE','FETCH','FILE','FILTER','FIRST','FLOAT','FOR','FOREIGN',
    'FREE_IT','FROM','FULL','FUNCTION','GDSCODE','GENERATOR','GEN_ID','GLOBAL','GRANT','GROUP',
    'GROUP_COMMIT_WAIT_TIME','HAVING','HOUR','IF', 'IIF', 'IN','INACTIVE','INDEX','INNER',
    'INPUT_TYPE','INSERT','INSERTING','INT','INTEGER','INTO','IS','ISOLATION',
    'JOIN','KEY','LAST','LEAVE','LEFT','LENGTH','LEVEL','LIKE','LOGFILE',
    'LOG_BUFFER_SIZE','LONG','MANUAL','MAX','MAXIMUM_SEGMENT','MERGE','MESSAGE',
    'MIN','MINUTE','MODULE_NAME','MONTH','NAMES','NATIONAL','NATURAL','NCHAR', 'NEXT', 'NO',
    'NOT','NULLIF','NULL','NULLS','LOCK','NUMERIC','NUM_LOG_BUFFERS','OF','ON','ONLY',
    'OPEN','OPTION','OR','ORDER','OUTER','OUTPUT_TYPE','OVERFLOW','PAGE','PAGES',
    'PAGE_SIZE','PARAMETER','PASSWORD','PERCENT','PLAN','POSITION','POST_EVENT','PRECISION',
    'PRIMARY','PRIVILEGES','PROCEDURE','PROTECTED','RAW_PARTITIONS','RDB$DB_KEY',
    'PRESERVE','READ','REAL','RECORD_VERSION','RECREATE','REFERENCES','RELEASE','RESERV',
    'RESERVING', 'RESTART','RESTRICT','RETAIN','RETURNING_VALUES','RETURNS','REVOKE','RIGHT',
    'ROLE','ROLLBACK','ROW_COUNT','ROWS','SAVEPOINT', 'SCALAR_ARRAY', 'SCHEMA','SECOND','SEGMENT',
    'SELECT','SEQUENCE','SET','SHADOW','SHARED','SINGULAR','SIZE','SKIP','SMALLINT','SNAPSHOT',
    'SOME','SORT','SQLCODE','STABILITY','STARTING','STARTS','STATEMENT','STATISTICS',
    'SUBSTRING','SUB_TYPE','SUM','SUSPEND','TABLE','TEMPORARY','THEN','TIES','TIME','TIMESTAMP','TO',
    'TRANSACTION','TRIGGER','TRUE','TYPE','UNCOMMITTED','UNION','UNIQUE','UPDATE','UPDATING',
    'UPPER','USER','USING','VALUE','VALUES','VARCHAR','VARIABLE','VARYING','VIEW',
    'WAIT','WEEKDAY','WHEN','WHERE','WHILE','WITH','WORK','WRITE','YEAR','YEARDAY');

   SQLComp: array[0..14] of TSQLCompInfo =
...
const
  ...
  // functions
  InterbaseFunctions = 'AVG,CAST,COUNT,GEN_ID,MAX,MIN,SUM,UPPER';
  ...
  InterbaseKW =
    'ACTION,ACTIVE,ADD,ADMIN,AFTER,ALL,ALTER,AND,ANY,AS,ASC,ASCENDING,AT,AUTO,'+
    'BACKUP,BASE_NAME,BEFORE,BEGIN,BETWEEN,BREAK,BY,CACHE,CASCADE,CASE,'+
    'CHECK,CHECK_POINT_LENGTH,CLOSE,COALESCE,COLLATE,COLUMN,COMMIT,COMMITTED,'+
    'COMPUTED,CONDITIONAL,CONSTRAINT,CONTAINING,CREATE,CSTRING,CURRENT,'+
    'CURRENT_CONNECTION,CURRENT_DATE,CURRENT_ROLE,CURRENT_TIME,CURRENT_TIMESTAMP,'+
    'CURRENT_TRANSACTION,CURRENT_USER,CURSOR,DATABASE,DAY,DEBUG,DEC,DECLARE,'+
    'DEFAULT,DELETE,DELETING,DESC,DESCENDING,DESCRIPTOR,DIFFERENCE,DISTINCT,DO,'+
    'DOMAIN,DROP,ELSE,END,ENTRY_POINT,ESCAPE,EXCEPTION,EXECUTE,EXISTS,EXIT,EXTERNAL,'+
    'EXTRACT,FALSE,FETCH,FILE,FILTER,FIRST,FOR,FOREIGN,FREE_IT,FROM,FULL,FUNCTION,GDSCODE,'+
    'GENERATOR,GLOBAL,GRANT,GROUP,GROUP_COMMIT_WAIT_TIME,HAVING,HOUR,IF,IN,INACTIVE,'+
    'INDEX,INNER,INPUT_TYPE,INSERT,INSERTING,INT,INTO,IS,ISOLATION,JOIN,KEY,'+
    'LAST,LEAVE,LEFT,LENGTH,LEVEL,LIKE,LOGFILE,LOG_BUFFER_SIZE,LONG,MANUAL,'+
    'MAXIMUM_SEGMENT,MERGE,MESSAGE,MINUTE,MODULE_NAME,MONTH,NAMES,NATIONAL,NATURAL,'+
    'NCHAR,NO,NOT,NULLIF,NULL,NULLS,LOCK,NUM_LOG_BUFFERS,OF,ON,ONLY,OPEN,OPTION,'+
    'OR,ORDER,OUTER,OUTPUT_TYPE,OVERFLOW,PAGE,PAGES,PAGE_SIZE,PARAMETER,PASSWORD,'+
    'PERCENT,PLAN,POSITION,POST_EVENT,PRECISION,PREPARE,PRESERVE,PRIMARY,PRIVILEGES,PROCEDURE,PROTECTED,'+
    'RAW_PARTITIONS,RDB$DB_KEY,READ,REAL,RECORD_VERSION,RECREATE,REFERENCES,RELEASE,'+
    'RESERV,RESERVING,RESTRICT,RETAIN,RETURNING_VALUES,RETURNS,REVOKE,RIGHT,'+
    'ROLE,ROLLBACK,ROW_COUNT,ROWS,SAVEPOINT,SCHEMA,SECOND,SEGMENT,SELECT,'+
    'SET,SHADOW,SHARED,SINGULAR,SIZE,SKIP,SNAPSHOT,SOME,SORT,SQLCODE,STABILITY,TIES,'+
    'STARTING,STARTS,STATEMENT,STATISTICS,SUBSTRING,SUB_TYPE,SUSPEND,TABLE,TEMPORARY,THEN,'+
    'TO,TRANSACTION,TRIGGER,TRUE,TYPE,UNCOMMITTED,UNION,UNIQUE,UPDATE,UPDATING,USER,'+
    'USING,VALUE,VALUES,VARIABLE,VARYING,VIEW,WAIT,WEEKDAY,WHEN,WHERE,WHILE,'+
    'WITH,WORK,WRITE,YEAR,YEARDAY';
