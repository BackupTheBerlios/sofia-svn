...
(*                                                                              *)
(********************************************************************************)
(* $Id: jvuiblib.pas,v 1.44 2005/03/21 22:07:35 progdigy Exp $                  *)
(********************************************************************************)
...
(******************************************************************************)
  ...
  private
    ...
    function GetWorkItem(const Index: Integer): Pointer;
    procedure AddPage;
  ...
  public
    ...
    property WorkListSize : Integer read FItemSize;
    property WorkList[const Index: Integer]: Pointer read GetWorkItem;
    property Count: Integer read GetWorkCount;
  end;
...
{ TSQLResult }
    ...
    begin
      if FFetchBlobs then
        for i := 0 to FMemoryPool.Count - 1 do
          FreeBlobs(FMemoryPool.WorkList[i]);
      FMemoryPool.Free;
      FMemoryPool := nil;
    end;
  end;
  ...
  begin
    ...
    begin
      Move(FMemoryPool.WorkList[Index]^, FDataBuffer^, FDataBufferLength);
  ...
  begin
    ...
    for i := 0 to Count - 1 do
    begin
      Stream.Write(FMemoryPool.WorkList[i]^, FDataBufferLength);
      for j := 0 to Length(FBlobsIndex) - 1 do
      begin
        BlobData := Pointer(Integer(FMemoryPool.WorkList[I]) + (integer(GetDataQuadOffset(FBlobsIndex[J])) - Integer(FDataBuffer)));
...
{ TMemoryPool }
  ...
  function TMemoryPool.GetWorkCount: Integer;
  begin
    Result := FWorkList.Count;
  end;

  function TMemoryPool.GetWorkItem(const Index: Integer): Pointer;
  begin
    Result := FWorkList.Items[Index];
  end;
...
{ TSQLParams }
  ...
  begin
    if not FindParam(name, Result) then
      Exception.CreateFmt(EUIB_PARAMSTRNOTFOUND, [name]);
  end;
